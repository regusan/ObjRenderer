...
void FPSCamera::OnUpdateInput(const XEvent &event)
{
    ...
    case KeyPress:
        // キーが押された場合
        {
            ...
            {
            case XK_w:
                SetPosition(this->location - GetForwardVector(this->mat.inverse()).head<3>());
                break;
            case XK_s:
                SetPosition(this->location + GetForwardVector(this->mat.inverse()).head<3>());
                break;
            case XK_a:
                SetPosition(this->location + GetRightVector(this->mat.inverse()).head<3>());
                break;
            case XK_d:
                SetPosition(this->location - GetRightVector(this->mat.inverse()).head<3>());
                break;
            case XK_e:
                SetPosition(this->location + GetUpVector(this->mat.inverse()).head<3>());
                break;
            case XK_q:
                SetPosition(this->location - GetUpVector(this->mat.inverse()).head<3>());
                break;

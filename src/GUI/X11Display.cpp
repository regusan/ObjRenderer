...
void X11Display::updateWindow(RenderTarget &renderTarget)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            unsigned char r = static_cast<unsigned char>(clamp(color.x() * 255.0, 0.0, 255.0));
            unsigned char g = static_cast<unsigned char>(clamp(color.y() * 255.0, 0.0, 255.0));
            unsigned char b = static_cast<unsigned char>(clamp(color.z() * 255.0, 0.0, 255.0));
